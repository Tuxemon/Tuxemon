...
class Economy:
    ...
    def get_item_field(self, item_slug: str, field: str) -> int:
        """
        ...
        Returns:
            The value of the field if found, otherwise raise ValueError.
        """
        item = self.get_item(item_slug)
        if item and hasattr(item, field):
            return int(getattr(item, field))
        raise ValueError(f"{item_slug} {field} doesn't exist")
    ...
    def get_monster_field(self, monster_name: str, field: str) -> int:
        """
        ...
        raise ValueError(f"{monster_name} {field} doesn't exist")
    ...
    def add_economy_to_npc(
        ...
        for item in items:
            if isinstance(item, Item):
                character.item_boxes.add_item(self.model.slug, item)
            else:
                character.monster_boxes.add_monster(self.model.slug, item)
