...
- Build scripts now run `scripts/prepare_web_build.py`, which removes unsupported audio formats (`.mp3`, `.flac`, `.m4a`, `.aac`, `.wma`, `.aiff`, `.alac`) from the temporary `web/` copy before packaging, and also pass `--disable-sound-format-error`.
