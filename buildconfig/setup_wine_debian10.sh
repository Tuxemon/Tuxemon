...
[ -e python-3.10.0-amd64.exe ] || wget -q https://www.python.org/ftp/python/3.10.0/python-3.10.0-amd64.exe
rm -rf ~/.wine
wineboot -u
xvfb-run winetricks win10
xvfb-run wine python-3.10.0-amd64.exe /quiet InstallAllUsers=1 PrependPath=1 Include_test=0 SimpleInstall=1
